...
println_symbol = PrintlnToken(new_tid())
if_symbol = IfToken(new_tid())
assign_symbol = AssignToken(new_tid())
while_symbol = WhileToken(new_tid())
let_symbol = LetToken(new_tid())
symbol_table = {"println":println_symbol, "if":if_symbol, "assign":assign_symbol,
                "while":while_symbol, "let":let_symbol}
...
def parse_token(token):
    ...
    if string_pattern.match(token):
        ...
        return println_symbol 
    elif ops.match(token):
        return OpToken(new_tid(), token)
    else:
        new_symbol = Token(new_tid(), token)
        symbol_table[token] = new_symbol
        return new_symbol
